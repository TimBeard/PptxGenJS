 ...
 */
export function genTableToSlides(pptx: PptxGenJS, tableElement: HTMLTableElement, options: TableToSlidesProps = {}, masterSlide?: SlideLayout): void {
	const opts = options || {}
	...
	if (firstRowCells.length === 0) firstRowCells = tableElement.querySelectorAll('tr:first-child td')
	...
	tableParts.forEach(part => {
		tableElement.querySelectorAll(`${part} tr`).forEach((row: Element) => {
